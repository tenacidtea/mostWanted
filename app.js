...
function getDescendants(foundPersonObject, arrayOfPeopleObjects, descendantsObject = []){
    let foundPersonsId = foundPersonObject.map(pluck => pluck.id);
    for(let index = 0; index < arrayOfPeopleObjects.length; index++){
        if(foundPersonsId == arrayOfPeopleObjects[index].parents[0]){
            descendantsObject.push(arrayOfPeopleObjects[index]);
        }
        else if(foundPersonsId == arrayOfPeopleObjects[index].parents[1]){
            descendantsObject.push(arrayOfPeopleObjects[index]);
            }
        }
    return getDescendants(foundPersonObject, arrayOfPeopleObjects, descendantsObject);
}    
...
function getDescendants(foundPersonObject, arrayOfPeopleObjects, descendantsObject = []){
    let foundPersonsId = foundPersonObject.map(pluck => pluck.id);
    for(let index = 0; index < arrayOfPeopleObjects.length; index++){
// Find out why this line doesn't work.  if(foundPersonsId === arrayOfPeopleObjects[index].parents[0] || arrayOfPeopleObjects[index].parents[1]){
            descendantsObject.push(arrayOfPeopleObjects[index]);
        }
    }
    return getDescendants(foundPersonObject, arrayOfPeopleObjects, descendantsObject);
...
/*
 function getDescendants(foundPersonObject, arrayOfPeopleObjects){
    for(let index = 0; index < arrayOfPeopleObjects.length; index++){
